...
const STAR_ARCHETYPES = [
	{ ramp: ':%$c1*uI1', planeScale: [3, 3, 3], drift: [0.03, 0.04, 0.01], radius: 0.8 },   // red dwarf
	{ ramp: 'g:*yT1~nI', planeScale: [3, 3, 3], drift: [0.03, 0.03, 0.02], radius: 1.05 },  // orange
	{ ramp: 'uL/~nA~zj~~~', planeScale: [2.5, 4, 2.5], drift: [0.02, 0.03, 0.01], radius: 1.3 }, // yellow (Sol)
	{ ramp: 'ywl~~~~~~', planeScale: [3, 3, 3], drift: [0.04, 0.04, 0.02], radius: 1.55 },  // white
	{ ramp: 'Lc~gp~z|~', planeScale: [3, 3, 3], drift: [0.04, 0.05, 0.02], radius: 1.85 }   // blue-white
]
...
function GenerateGalaxy(ctx) {
	...
	for (let gz = 0; gz < GALAXY_GRID; gz++) {
		for (let gx = 0; gx < GALAXY_GRID; gx++) {
			const x = (gx - half) * GALAXY_SPACING + (Mr() * 2 - 1) * GALAXY_JITTER
			const z = (gz - half) * GALAXY_SPACING + (Mr() * 2 - 1) * GALAXY_JITTER
...
function UpdateGalaxyMarkers(angle) {
	...
	if (selectedStarIndex >= 0) {
		...
		selectedMarker.update(V3(sel.x, 0.3, sel.z), sel.object.radius * 1.9, angle + PI)
	}
}

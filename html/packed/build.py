...
BUILD_LOG_HEADER = (
	...
	'Файл в `.gitignore` (локальный трек), но **не чистить** — это единственный\n'
	'способ видеть динамику бюджета между заходами. `build.py` только дописывает\n'
	'строки, шапку не трогает; если файл удалить — восстановится только эта шапка\n'
	'с вехами ниже, накопленные строки пропадут.\n\n'
	'## Вехи (реконструкция из docs/optimization.md, до автолога)\n\n'
	'| точка | zip, Б | дельта |\n'
	'|---|---:|---:|\n'
	'| до захода 1 | 14 853 | — |\n'
	'| заход 1 (алфавит через charCodeAt, ручной GLSL) | 14 619 | -234 |\n'
	'| заход 2 (дедуп vizColor, PushObjects) | 14 593 | -26 |\n'
	'| заход 3 (WebGL-энумы → числа, F32, имена камеры) | 14 467 | -126 |\n'
	'| + блоки «истинный масштаб» / звук / переделка UI | ~16 200 | +~1 700 |\n'
	'| заход 4 (дебаг-тулинг вне релиза, короткие CSS-имена) | ~15 830 | -366 |\n'
	'| заход 5 шаг 1 (`gl` — глобаль, `MeshCharVal(s,i)`) | ~15 690 | -140 |\n\n'
	'## Автолог\n\n'
